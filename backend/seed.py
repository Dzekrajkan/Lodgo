    ...
    try:
        ...
        if not second_user:
            second_user = models.User(
                ...
                password_hash=auth_service.hash_password("alex1234"),
                is_verified=True,
                is_admin=False,
            )
            db.add(second_user)
        ...
        if not facilities:
            ...
            db.commit()
            facilities = [wifi, pool, gym, parking, restaurant]
        ...
        if not hotels:
            ...
            hotel_1, hotel_2 = hotels[0], hotels[1]
        ...
        if not db.query(models.Service).first():
            db.add_all([
                models.Service(hotel_id=hotel_1.id, name="Airport Transfer", price=40),
                models.Service(hotel_id=hotel_1.id, name="Breakfast Buffet", price=15),
                models.Service(hotel_id=hotel_2.id, name="Spa Access", price=30),
                models.Service(hotel_id=hotel_2.id, name="Mountain Guide Tour", price=50),
                models.Service(hotel_id=hotel_2.id, name="Ski Equipment Rental", price=35),
            ])
            db.commit()
        ...
        if not db.query(models.Booking).first():
            ...
            booking_1 = models.Booking(
                user_id=second_user.id, hotel_id=hotel_1.id, room_id=room_1.id,
                guest_first_name="Alex", guest_last_name="Johnson", guest_email="alex@example.com",
                date_from=now - timedelta(days=10), date_to=now - timedelta(days=7),
                total_price=360, status=models.BookingStatus.completed,
            )
            booking_2 = models.Booking(
                user_id=second_user.id, hotel_id=hotel_1.id, room_id=room_1.id,
                guest_first_name="Alex", guest_last_name="Johnson", guest_email="alex@example.com",
                date_from=now - timedelta(days=20), date_to=now - timedelta(days=18),
                total_price=240, status=models.BookingStatus.completed,
            )
            booking_3 = models.Booking(
                user_id=second_user.id, hotel_id=hotel_2.id, room_id=room_2.id,
                guest_first_name="Alex", guest_last_name="Johnson", guest_email="alex@example.com",
                date_from=now - timedelta(days=15), date_to=now - timedelta(days=12),
                total_price=420, status=models.BookingStatus.completed,
            )
            ...
            db.add_all([
                models.Review(user_id=second_user.id, hotel_id=hotel_1.id, booking_id=bookings[0].id, rating=3, comment="Good location but the room could be cleaner."),
                models.Review(user_id=second_user.id, hotel_id=hotel_1.id, booking_id=bookings[1].id, rating=5, comment="Excellent stay! Friendly staff and amazing view."),
                models.Review(user_id=second_user.id, hotel_id=hotel_2.id, booking_id=bookings[2].id, rating=2, comment="Nice location but the service was disappointing."),
            ])
            db.commit()
            print("Bookings and reviews created")
